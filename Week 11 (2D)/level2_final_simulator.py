...
def getRoute(f):
    ...
    #newRoute=newRoute+['stop']
    #newRoute=['turnRight','turnLeft','goStraight','goStraight','stop']
...
# inputFile=open('level2_2.inp','r')
# routes=getRoute(inputFile)
# print routes
routes=[[],['turnLeft','turnRight']]
class MySMClass(sm.SM):
    ...
    def getNextValues(self, state, inp):
        ...
        if currentState=='forward':
            ...
            elif (data[0]>=2 and data[1]>=0.8) or (data[3]>=2 and data[4]>=0.8):
        ...
        if currentState=='junction':
                ...
                #    outputTime=time.strftime("%H:%M:%S|%d/%b/%Y", time.gmtime())
                    ...
                    state['state']=route[num]
                    output=io.Action(fvel,0.0)
                    state['previousAngle']=currentAngle1
